...
use crate::settings::{
    get_settings, AppSettings, CantoneseOutputStyle, ChineseOutputScript, OverlayStyle,
...
fn should_run_cloud_post_process(requested: bool, strict_offline_mode: bool) -> bool {
    requested && !strict_offline_mode
}
...
fn contains_han_script(text: &str) -> bool {
...
fn looks_like_cantonese(text: &str) -> bool {
    const CANTONESE_MARKERS: [&str; 24] = [
        "唔", "冇", "嘅", "喺", "咗", "啲", "佢", "哋", "嚟", "咁", "噉", "乜", "嘢", "畀", "俾",
        "仲", "而家", "點解", "点解", "得閒", "得闲", "呢個", "呢个", "嗰個",
    ];
    CANTONESE_MARKERS.iter().any(|marker| text.contains(marker))
}

/// Conservative, deterministic lexical normalization. This deliberately does
/// not use a generative model: preserving meaning is more important than making
/// every sentence read like edited prose.
fn maybe_formalize_cantonese(
    selected_language: &str,
    style: CantoneseOutputStyle,
    transcription: &str,
) -> Option<String> {
    if style != CantoneseOutputStyle::Written
        || (selected_language != "yue" && !looks_like_cantonese(transcription))
    {
        return None;
    }

    // Longest/specific forms precede their component words.
    const REPLACEMENTS: [(&str, &str); 27] = [
        ("唔得閒", "没空"),
        ("唔得闲", "没空"),
        ("點解", "为什么"),
        ("点解", "为什么"),
    ...
    let mut normalized = transcription.to_string();
    for (cantonese, written) in REPLACEMENTS {
        normalized = normalized.replace(cantonese, written);
    }
    (normalized != transcription).then_some(normalized)
}

async fn maybe_convert_chinese_script(
    output_script: ChineseOutputScript,
    transcription: &str,
) -> Option<String> {
    if !contains_han_script(transcription) {
        return None;
    }

    debug!(
        "Starting Chinese script conversion using OpenCC: {:?}",
        output_script
    );

    let config = match output_script {
        ChineseOutputScript::Simplified => BuiltinConfig::Tw2sp,
        // Hong Kong Traditional is the correct default for the target accent
        // and vocabulary, rather than generic Taiwan Traditional.
        ChineseOutputScript::Traditional => BuiltinConfig::S2hk,
    };
...
pub(crate) async fn process_transcription_output(
    ...
    if let Some(written_text) = maybe_formalize_cantonese(
        &settings.selected_language,
        settings.cantonese_output_style,
        transcription,
    ) {
        final_text = written_text;
    }
    if let Some(converted_text) =
        maybe_convert_chinese_script(settings.chinese_output_script, &final_text).await
    {
        final_text = converted_text;
    }

    if should_run_cloud_post_process(post_process, settings.strict_offline_mode) {
        ...
        if post_process && settings.strict_offline_mode {
            info!("Cloud post-processing blocked by strict offline privacy mode");
        }
...
impl ShortcutAction for TranscribeAction {
    fn start(&self, app: &AppHandle, binding_id: &str, _shortcut_str: &str) {
        ...
        let model_supports_streaming = selected_model_info
            ...
            .unwrap_or(false);
        let sense_voice_live_preview = settings.overlay_style == OverlayStyle::Live
            && selected_model_info
                .as_ref()
                .is_some_and(|model| matches!(model.engine_type, EngineType::SenseVoice));
        let live_text_requested = model_supports_streaming || sense_voice_live_preview;
        let vad_policy = if !settings.vad_enabled {
            VadPolicy::Disabled
        } else if live_text_requested {
            VadPolicy::Streaming
        } else {
            VadPolicy::Offline
        };
        if live_text_requested {
            tm.start_stream();
        }
        ...
        match settings.overlay_style {
            OverlayStyle::Live if live_text_requested => utils::show_streaming_overlay(app),
...
mod tests {
    use super::{
        complete_unless_cancelled, contains_han_script, is_blank_transcription,
        looks_like_cantonese, maybe_formalize_cantonese, should_run_cloud_post_process,
        should_use_streaming_overlay, strip_think_block,
    };
    use crate::settings::{CantoneseOutputStyle, OverlayStyle};
    use std::future;
    ...
    #[test]
    fn strict_offline_mode_is_a_hard_cloud_gate() {
        assert!(!should_run_cloud_post_process(true, true));
        assert!(!should_run_cloud_post_process(false, false));
        assert!(should_run_cloud_post_process(true, false));
    }

    #[test]
    fn detects_cantonese_markers_without_misclassifying_english() {
        assert!(looks_like_cantonese("我今日唔得閒"));
        assert!(!looks_like_cantonese("I cannot join today"));
        assert!(contains_han_script("普通话 and English"));
    }

    #[test]
    fn written_cantonese_mode_is_conservative_and_optional() {
        assert_eq!(
            maybe_formalize_cantonese("auto", CantoneseOutputStyle::Written, "我今日唔得閒"),
            Some("我今天没空".to_string())
        );
        assert_eq!(
            maybe_formalize_cantonese("yue", CantoneseOutputStyle::Spoken, "我今日唔得閒"),
            None
        );
        assert_eq!(
            maybe_formalize_cantonese("auto", CantoneseOutputStyle::Written, "普通话保持原样"),
            None
        );
    }
}
