...
pub fn apply_custom_words(text: &str, custom_words: &[String], threshold: f64) -> String {
    ...
    // Entries using `heard→wanted` are deterministic replacements. This makes
    // Chinese names and domain terms correctable even though the fallback
    // phonetic matcher is intentionally ASCII-only.
    let mut exact_corrected = text.to_string();
    let fuzzy_words: Vec<String> = custom_words
        .iter()
        .filter_map(|entry| {
            if let Some((heard, wanted)) = entry.split_once('→') {
                let heard = heard.trim();
                let wanted = wanted.trim();
                if !heard.is_empty() && !wanted.is_empty() {
                    exact_corrected = exact_corrected.replace(heard, wanted);
                }
                None
            } else {
                Some(entry.clone())
    ...
    if fuzzy_words.is_empty() {
        return exact_corrected;
    }

    // Pre-compute normalized comparison keys to avoid repeated allocations.
    let custom_word_match_keys: Vec<CustomWordMatchKey> = fuzzy_words
        .iter()
        .enumerate()
        .flat_map(|(index, word)| build_custom_word_match_keys(word, index))
        .collect();

    let words: Vec<&str> = exact_corrected.split_whitespace().collect();
    ...
    while i < words.len() {
        ...
        // Consider n-grams up to four words and choose the closest match. Four
        // covers letter-by-letter forms such as "Chat G P T". A
        ...
        for n in (1..=4).rev() {
            ...
            if let Some((replacement, score)) =
                find_best_match(&ngram, &fuzzy_words, &custom_word_match_keys, threshold)
            {
...
fn collapse_stutters(text: &str) -> String {
    ...
    collapse_cjk_stutters(&result.join(" "))
}

fn collapse_cjk_stutters(text: &str) -> String {
    ...
    while index < chars.len() {
        ...
        // Collapse three or more direct repetitions: “我我我觉得”. Two
        // repetitions are retained because they can be intentional emphasis.
        let mut direct_end = index + 1;
        while direct_end < chars.len() && chars[direct_end] == current {
            direct_end += 1;
        }
        if direct_end - index >= 3 {
            result.push(current);
            index = direct_end;
            continue;
        }

        // Also handle punctuation-separated hesitations: “我、我、我觉得”.
        let mut cursor = index + 1;
        let mut repetitions = 1;
        let mut consumed_end = cursor;
        loop {
            let separator_start = cursor;
            while cursor < chars.len() && is_stutter_separator(chars[cursor]) {
                cursor += 1;
            }
            if cursor == separator_start || cursor >= chars.len() || chars[cursor] != current {
                break;
            }
            repetitions += 1;
            cursor += 1;
...
fn is_stutter_separator(c: char) -> bool {
...
pub fn normalize_transcription_output(text: &str) -> String {
    ...
    // Conservative local corrections for recurring SenseVoice code-switching
    // artifacts. These run on-device and are intentionally narrow so normal
    // prose is not broadly rewritten.
    for (heard, wanted) in [
        ("five coding", "vibe coding"),
        ("Five Coding", "vibe coding"),
        ("vibe coading", "vibe coding"),
        ("点」范网", "的饭碗"),
        ("点范网", "的饭碗"),
        ("点范碗", "的饭碗"),
        ("的范网", "的饭碗"),
    ] {
        normalized = normalized.replace(heard, wanted);
    }
...
mod tests {
    ...
    #[test]
    fn collapses_clear_cjk_stutters_but_keeps_emphasis() {
        assert_eq!(
            normalize_transcription_output("我、我、我觉得可以"),
            "我觉得可以"
        );
        assert_eq!(
            normalize_transcription_output("我我我觉得可以"),
            "我觉得可以"
        );
        assert_eq!(
            normalize_transcription_output("非常非常重要"),
            "非常非常重要"
        );
    }

    #[test]
    fn fixes_reported_mixed_language_phrase_artifacts() {
        assert_eq!(
            normalize_transcription_output("我觉得five coding真的砸了程序员和产品经理点」范网"),
            "我觉得vibe coding真的砸了程序员和产品经理的饭碗"
        );
        assert_eq!(normalize_transcription_output("点范网"), "的饭碗");
    }
    ...
    #[test]
    fn test_apply_custom_words_supports_exact_cjk_replacement_rules() {
        let text = "陈小明在新鸿基开会，之后讲 Chat G P T";
        let custom_words = vec![
            "陈晓明→陈小明".to_string(),
            "新红基→新鸿基".to_string(),
            "ChatGPT".to_string(),
        ...
        let result = apply_custom_words(
            &text.replace("陈小明", "陈晓明").replace("新鸿基", "新红基"),
            &custom_words,
            0.5,
        );
        assert_eq!(result, "陈小明在新鸿基开会，之后讲 ChatGPT");
    }
}
