...
pub fn paste(text: String, app_handle: AppHandle) -> Result<(), String> {
    ...
    let text = apply_terminal_period_preference(
        text,
        settings.append_terminal_period,
...
fn append_terminal_period(text: String) -> String {
    let trimmed = text.trim_end();
    if trimmed.is_empty() {
        return text;
    }

    let terminal = trimmed.chars().last().unwrap_or_default();
...
fn apply_terminal_period_preference(
    text: String,
    terminal_period_enabled: bool,
    spoken_punctuation_enabled: bool,
) -> String {
    // Remove a model-generated full stop before translating spoken punctuation.
    // This lets an explicitly dictated "句号"/"period" survive even when the
    // automatic terminal-period preference is disabled.
    let text = remove_terminal_period(text);
    let text = if spoken_punctuation_enabled {
        normalize_spoken_punctuation(&text)
    } else {
        text
    };

    if terminal_period_enabled {
        return append_terminal_period(text);
    }

    text
}

static ENGLISH_SPOKEN_PUNCTUATION: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r"(?i)\b(exclamation point|exclamation mark|question mark|full stop|semicolon|ellipsis|comma|period|colon)\b",
    )
    .unwrap()
});
static SPACE_BEFORE_PUNCTUATION: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\s+([，。？！、：；,.?!:;])").unwrap());
static SPACE_AFTER_CJK_PUNCTUATION: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"([，。？！、：；])\s+").unwrap());
static DUPLICATE_CHINESE_COMMAS: Lazy<Regex> = Lazy::new(|| Regex::new(r"，{2,}").unwrap());
static DUPLICATE_ENGLISH_COMMAS: Lazy<Regex> = Lazy::new(|| Regex::new(r",{2,}").unwrap());
static PUNCTUATION_BEFORE_SPOKEN_MARKER: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"[，。？！、：；,.?!:;]+\s*(__TRISCRIBE_SPOKEN_PUNCT_[0-9]+__)").unwrap()
});
static PUNCTUATION_AFTER_SPOKEN_MARKER: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(__TRISCRIBE_SPOKEN_PUNCT_[0-9]+__)\s*[，。？！、：；,.?!:;]+").unwrap()
});
...
fn normalize_spoken_punctuation(text: &str) -> String {
    ...
    for (spoken, marker) in [
        ("感叹号", "__TRISCRIBE_SPOKEN_PUNCT_0__"),
        ("感嘆號", "__TRISCRIBE_SPOKEN_PUNCT_0__"),
        ("惊叹号", "__TRISCRIBE_SPOKEN_PUNCT_0__"),
        ("驚嘆號", "__TRISCRIBE_SPOKEN_PUNCT_0__"),
        ("省略号", "__TRISCRIBE_SPOKEN_PUNCT_1__"),
        ("省略號", "__TRISCRIBE_SPOKEN_PUNCT_1__"),
        ("问号", "__TRISCRIBE_SPOKEN_PUNCT_2__"),
        ("問號", "__TRISCRIBE_SPOKEN_PUNCT_2__"),
        ("顿号", "__TRISCRIBE_SPOKEN_PUNCT_3__"),
        ("頓號", "__TRISCRIBE_SPOKEN_PUNCT_3__"),
        ("分号", "__TRISCRIBE_SPOKEN_PUNCT_4__"),
        ("分號", "__TRISCRIBE_SPOKEN_PUNCT_4__"),
        ("冒号", "__TRISCRIBE_SPOKEN_PUNCT_5__"),
        ("冒號", "__TRISCRIBE_SPOKEN_PUNCT_5__"),
        ("逗号", "__TRISCRIBE_SPOKEN_PUNCT_6__"),
        ("逗號", "__TRISCRIBE_SPOKEN_PUNCT_6__"),
        ("句号", "__TRISCRIBE_SPOKEN_PUNCT_7__"),
        ("句號", "__TRISCRIBE_SPOKEN_PUNCT_7__"),
    ] {
        normalized = normalized.replace(spoken, marker);
    }

    normalized = ENGLISH_SPOKEN_PUNCTUATION
        .replace_all(
            &normalized,
            |captures: &regex::Captures<'_>| match captures[0].to_ascii_lowercase().as_str() {
                "comma" => "__TRISCRIBE_SPOKEN_PUNCT_8__",
                "period" | "full stop" => "__TRISCRIBE_SPOKEN_PUNCT_9__",
                "question mark" => "__TRISCRIBE_SPOKEN_PUNCT_10__",
                "exclamation mark" | "exclamation point" => "__TRISCRIBE_SPOKEN_PUNCT_11__",
                "colon" => "__TRISCRIBE_SPOKEN_PUNCT_12__",
                "semicolon" => "__TRISCRIBE_SPOKEN_PUNCT_13__",
                "ellipsis" => "__TRISCRIBE_SPOKEN_PUNCT_14__",
                _ => unreachable!(),
            },
        )
        .to_string();

    // Recognition models sometimes add their own punctuation immediately next
    // to a spoken punctuation word. The explicitly dictated mark wins.
    normalized = PUNCTUATION_BEFORE_SPOKEN_MARKER
        .replace_all(&normalized, "$1")
        .to_string();
    normalized = PUNCTUATION_AFTER_SPOKEN_MARKER
        .replace_all(&normalized, "$1")
        .to_string();

    for (marker, punctuation) in [
        ("__TRISCRIBE_SPOKEN_PUNCT_0__", "！"),
        ("__TRISCRIBE_SPOKEN_PUNCT_1__", "……"),
        ("__TRISCRIBE_SPOKEN_PUNCT_2__", "？"),
        ("__TRISCRIBE_SPOKEN_PUNCT_3__", "、"),
        ("__TRISCRIBE_SPOKEN_PUNCT_4__", "；"),
        ("__TRISCRIBE_SPOKEN_PUNCT_5__", "："),
        ("__TRISCRIBE_SPOKEN_PUNCT_6__", "，"),
        ("__TRISCRIBE_SPOKEN_PUNCT_7__", "。"),
        ("__TRISCRIBE_SPOKEN_PUNCT_8__", ","),
        ("__TRISCRIBE_SPOKEN_PUNCT_9__", "."),
        ("__TRISCRIBE_SPOKEN_PUNCT_10__", "?"),
        ("__TRISCRIBE_SPOKEN_PUNCT_11__", "!"),
        ("__TRISCRIBE_SPOKEN_PUNCT_12__", ":"),
        ("__TRISCRIBE_SPOKEN_PUNCT_13__", ";"),
        ("__TRISCRIBE_SPOKEN_PUNCT_14__", "..."),
    ] {
        normalized = normalized.replace(marker, punctuation);
    }
    normalized = SPACE_BEFORE_PUNCTUATION
        .replace_all(&normalized, "$1")
        .to_string();
    normalized = SPACE_AFTER_CJK_PUNCTUATION
        .replace_all(&normalized, "$1")
        .to_string();
    normalized = DUPLICATE_CHINESE_COMMAS
        .replace_all(&normalized, "，")
        .to_string();
    normalized = DUPLICATE_ENGLISH_COMMAS
        .replace_all(&normalized, ",")
        .to_string();
...
fn remove_terminal_period(mut text: String) -> String {
    ...
    for (index, character) in content.char_indices().rev() {
        ...
        if character == '。' || character == '.' {
            // Preserve an English ellipsis rather than turning "..." into "..".
            if character == '.' && content[..index].ends_with('.') {
                return text;
            }
            text.replace_range(index..index + character.len_utf8(), "");
        }
        break;
    }
...
"#;
    ...
    #[test]
    fn terminal_period_uses_chinese_or_english_punctuation() {
        assert_eq!(append_terminal_period("你好".into()), "你好。");
        assert_eq!(append_terminal_period("hello world".into()), "hello world.");
        assert_eq!(
            append_terminal_period("我觉得 vibe coding 很好".into()),
            "我觉得 vibe coding 很好。"
        );
    }

    #[test]
    fn terminal_period_does_not_duplicate_existing_punctuation() {
        for value in ["已经有句号。", "question?", "感叹！", "省略……"] {
            assert_eq!(append_terminal_period(value.into()), value);
        }
    }

    #[test]
    fn disabled_terminal_period_removes_model_generated_periods() {
        assert_eq!(
            apply_terminal_period_preference("你好。".into(), false, true),
            "你好"
        );
        assert_eq!(
            apply_terminal_period_preference("hello world.".into(), false, true),
            "hello world"
        );
        assert_eq!(
            apply_terminal_period_preference("“你好。”".into(), false, true),
        ...
        assert_eq!(
            apply_terminal_period_preference("hello.  \n".into(), false, true),
            "hello  \n"
        );
    }

    #[test]
    fn disabled_terminal_period_preserves_other_end_punctuation() {
        for value in ["问题？", "question?", "感叹！", "Wait...", "省略……"] {
            assert_eq!(
                apply_terminal_period_preference(value.into(), false, true),
                value
            );
        }
    }
    ...
    #[test]
    fn spoken_chinese_punctuation_is_translated_for_paste() {
        assert_eq!(
            apply_terminal_period_preference(
                "如果你想要去那边逗号那你就要跟我过来句号".into(),
                false,
                true,
            ),
            "如果你想要去那边，那你就要跟我过来。"
        ...
        assert_eq!(
            apply_terminal_period_preference("你去唔去問號".into(), false, true),
            "你去唔去？"
        );
        assert_eq!(
            apply_terminal_period_preference(
                "如果你想去那边，逗号那你就来跟我这边吧句号。".into(),
                false,
                true,
            ),
            "如果你想去那边，那你就来跟我这边吧。"
    ...
    #[test]
    fn spoken_english_punctuation_is_translated_and_spaced() {
        assert_eq!(
            apply_terminal_period_preference(
                "If you want to go there comma follow me period".into(),
                false,
                true,
            ),
    ...
    #[test]
    fn spoken_period_survives_when_automatic_period_is_disabled() {
        assert_eq!(
            apply_terminal_period_preference("请跟我过来句号。".into(), false, true),
            "请跟我过来。"
        );
        assert_eq!(
            apply_terminal_period_preference("自动生成句号。".into(), false, true),
            "自动生成。"
        );
    }
