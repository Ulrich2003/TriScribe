...
impl TranscriptionManager {
    ...
    fn run_stream_worker(&self, rx: mpsc::Receiver<StreamCmd>, worker_id: u64) {
        ...
        // SenseVoice is a fast batch model rather than a native streaming
        // model. In Live overlay mode we provide a throttled, on-device
        // segmented preview while recording. Finalize deliberately returns
        // `None`: the normal batch path then re-runs the complete recording so
        // preview boundaries can never reduce final accuracy.
        if let LoadedEngine::SenseVoice(sense_voice) = &mut engine {
            let settings = get_settings(&self.app_handle);
            ...
            let finalize_reply = self.run_sense_voice_live_preview(sense_voice, rx, language);
    ...
    fn run_sense_voice_live_preview(
        &self,
        engine: &mut SenseVoiceModel,
        rx: mpsc::Receiver<StreamCmd>,
        language: Option<String>,
    ) -> Option<mpsc::Sender<Option<FinalizedStreamText>>> {
        const FIRST_PREVIEW_SAMPLES: usize = 2 * 16_000;
        const PREVIEW_INTERVAL_SAMPLES: usize = 24_000; // about 1.5 seconds
        const COMMIT_WINDOW_SAMPLES: usize = 16 * 16_000;
        ...
        self.stream_active.store(true, Ordering::Release);
        self.touch_activity();
        info!("SenseVoice on-device segmented live preview started");
        ...
        while let Ok(command) = rx.recv() {
            match command {
                StreamCmd::Feed(pcm) => {
                    ...
                    let due = window.len() >= FIRST_PREVIEW_SAMPLES
                        && window.len().saturating_sub(last_preview_at) >= PREVIEW_INTERVAL_SAMPLES;
                    if !due {
                        continue;
                    }

                    match engine.transcribe_with(&window, &params) {
                        Ok(result) => {
                            let preview = normalize_transcription_output(&result.text);
                            if !preview.is_empty() {
                                self.emit_stream_text(&committed, &preview);
                            }
                            last_preview_at = window.len();

                            if window.len() >= COMMIT_WINDOW_SAMPLES {
                                if !preview.is_empty() {
                                    if !committed.is_empty() {
                                        committed.push(' ');
                                    }
                                    committed.push_str(&preview);
                                    self.emit_stream_text(&committed, "");
                                }
                                window.clear();
                                last_preview_at = 0;
                            }
                        ...
                        Err(error) => {
                            warn!("SenseVoice live preview decode failed: {}", error);
                            last_preview_at = window.len();
                        }
                    }
                }
            ...
            let transcribe_result = catch_unwind(AssertUnwindSafe(|| -> Result<String> {
                match &mut engine {
                    ...
                    LoadedEngine::SenseVoice(sense_voice_engine) => {
                        ...
                        // SenseVoice is optimized for short utterances. Bound
                        // each inference window so multi-minute dictation does
                        // not create a single oversized encoder pass. Capture
                        // VAD has already removed non-speech frames; chunks are
                        // merged in order without any cloud-side processing.
                        const SENSE_VOICE_CHUNK_SAMPLES: usize = 28 * 16_000;
                        let mut parts = Vec::new();
                        for chunk in audio.chunks(SENSE_VOICE_CHUNK_SAMPLES) {
                            let text = sense_voice_engine
                                .transcribe_with(chunk, &params)
                                .map(|result| result.text)
                                .map_err(|error| {
                                    anyhow::anyhow!("SenseVoice transcription failed: {}", error)
                                })?;
