...
pub struct AppSettings {
    ...
    /// Output orthography is independent from recognition language so automatic
    /// Mandarin/Cantonese/English detection remains available.
    #[serde(default)]
    pub chinese_output_script: ChineseOutputScript,
    /// Whether Cantonese should stay verbatim or receive conservative local
    /// lexical normalization into written Chinese.
    #[serde(default)]
    pub cantonese_output_style: CantoneseOutputStyle,
    ...
    /// Hard privacy gate. When enabled, no remote LLM post-processing request
    /// may be made, even if a post-processing shortcut is invoked.
    #[serde(default = "default_strict_offline_mode")]
    pub strict_offline_mode: bool,
    ...
    #[serde(default)]
    pub append_terminal_period: bool,
...
fn default_overlay_style() -> OverlayStyle {
    // Linux hides the overlay by default; other platforms show the compact
    // recording indicator without live partial transcription.
...
fn default_strict_offline_mode() -> bool {
    true
}
...
pub fn get_default_settings() -> AppSettings {
    ...
    AppSettings {
        ...
        chinese_output_script: ChineseOutputScript::default(),
        cantonese_output_style: CantoneseOutputStyle::default(),
        ...
        strict_offline_mode: default_strict_offline_mode(),
...
fn apply_settings_migrations(
    ...
    // other position had it visible → Minimal. The position enum no longer has a
...
mod tests {
    ...
    fn empty_store_parses_with_defaults() {
        ...
        assert!(settings.spoken_punctuation_enabled);
