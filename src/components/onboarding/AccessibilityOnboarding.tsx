...
const AccessibilityOnboarding: React.FC<AccessibilityOnboardingProps> = ({
  ...
  const handleOpenAccessibilitySettings = async () => {
    ...
    try {
      ...
      console.warn("Failed to open Accessibility settings directly:", error);
      await requestAccessibilityPermission();
    } finally {
      startPolling();
    }
  ...
  const handleCheckAgain = async () => {
    ...
    try {
      ...
      console.error("Failed to recheck macOS permissions:", error);
      setPermissions((prev) => ({ ...prev, accessibility: "needed" }));
      toast.error(t("onboarding.permissions.errors.checkFailed"));
    }
  };
                ...
                {permissions.accessibility === "granted" ? (
                  ...
                  <div className="space-y-3">
                    ...
                    <p className="text-xs leading-5 text-text/60">
                      {t("onboarding.permissions.accessibility.waitingHint")}
                    </p>
                    <div className="flex flex-wrap gap-2">
                      <button
                        onClick={handleOpenAccessibilitySettings}
                        className="inline-flex items-center gap-1.5 rounded-lg border border-mid-gray/40 px-3 py-2 text-xs font-medium text-text transition-colors hover:bg-white/10"
                      >
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 rounded-lg bg-logo-primary px-3 py-2 text-xs font-medium text-white transition-colors hover:bg-logo-primary/90"
                      >
                      ...
                      >
                        ...
                        {t("onboarding.permissions.restartApp")}
                      </button>
                    </div>
                  </div>
                ) : (
