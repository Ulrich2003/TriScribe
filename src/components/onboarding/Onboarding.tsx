...
const Onboarding: React.FC<OnboardingProps> = ({
  ...
  // TriScribe is deliberately focused on Mandarin, Hong Kong Cantonese, and
  // English. Feature SenseVoice alone so first-time users cannot accidentally
  // choose an English-only model; all advanced alternatives remain under
  // "Show all".
  const { downloadable, topPicks, otherRecommended, rest } = useMemo(() => {
    ...
    const primary = downloadable.find(
      (model: ModelInfo) => model.id === "sense-voice-int8",
    );
    const remaining = downloadable.filter(
      (model: ModelInfo) => model.id !== primary?.id,
    );
    const recommended = remaining.filter(
      (model: ModelInfo) => model.is_recommended,
    );
    return {
      downloadable,
      topPicks: primary ? [primary] : recommended.slice(0, 2),
      otherRecommended: primary ? [] : recommended.slice(2),
      rest: primary
        ? remaining
        : remaining.filter((model: ModelInfo) => !model.is_recommended),
    };
  }, [models]);
