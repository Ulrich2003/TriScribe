...
const TRISCRIBE_LANGUAGE_CODES = new Set(["auto", "en", "zh", "yue"]);
...
// TriScribe intentionally presents only the three languages in the product
// brief. Chinese output script is a separate setting so auto detection remains
// available while the user chooses Simplified or Hong Kong Traditional text.
export const SELECTABLE_LANGUAGES: Language[] = LANGUAGES.filter((language) =>
  TRISCRIBE_LANGUAGE_CODES.has(language.value),
);
